...
template <typename T>
void assert_eq(const char *trace, const char *name1, T val1, const char *name2,
		T val2) {
	if (val1 != val2) {
...
void assert_ptr_not_null(const char *trace, const char *name, const T *p) {
	if (nullptr == p) {
		throw runtime_error(str_t(trace) + "pointer to " + name +
				" is NULL");
	}
}

template <typename T>
void check_array(const char *trace, const char *name, const T *p, size_t size,
		bool interleaved = false) {
	assert_ptr_not_null(trace, name, p);
	str_t n = str_t(name) + " size";
	assert_gt0(trace, n.c_str(), size);
	if (interleaved && is_odd(size)) {
		throw runtime_error(str_t(trace) +
				" must be even if interleaved");
	}
}

template <typename T1, typename T2>
void check_array_pair(const char *trace, const char *name1, const T1 *p1,
		size_t size1, const char *name2, const T2 *p2,
		size_t size2, bool interleaved = false) {
...
inline void check_code_width(const char *trace, int n) {
	if (n < k_abs_min_code_width || k_abs_max_code_width < n) {
		throw runtime_error(
				str_t(trace) +
				"resolution outside absolute code width limits : [" +
