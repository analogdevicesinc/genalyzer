...
private:
	...
	virtual bool
	equals_impl(const fourier_analysis_component &that) const = 0;
...
private:
	...
	equals_impl(const fourier_analysis_component &that_obj) const override {
		bool equal = (this->type == that_obj.type) &&
				(this->tag == that_obj.tag);
		if (!equal) {
			return false;
		}
...
struct fa_fixed_tone final : public fourier_analysis_component {
	...
	fa_fixed_tone(FACompTag tag, const str_t &_freq, int _ssb) :
			fourier_analysis_component(FACompType::FixedTone, tag), freq(_freq), ssb(_ssb) {
	}
...
struct fa_max_tone final : public fourier_analysis_component {
	static pointer create(FACompTag tag, const str_t &center,
	...
	fa_max_tone(FACompTag tag, const str_t &_center, const str_t &_width,
			int _ssb) :
			fourier_analysis_component(FACompType::MaxTone, tag), center(_center), width(_width), ssb(_ssb) {
	}
...
private:
	...
	equals_impl(const fourier_analysis_component &that_obj) const override {
		...
		return (this->center == that.center) &&
				(this->width == that.width) && (this->ssb == that.ssb);
	}
...
struct fa_wo_tone final : public fourier_analysis_component {
	...
	fa_wo_tone(int _ssb) :
			fourier_analysis_component(FACompType::WOTone,
					FACompTag::Noise),
			ssb(_ssb) {
	}
