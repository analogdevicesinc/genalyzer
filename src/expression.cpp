...
enum class OpArity { Unary,
	Binary };

enum class OpAssoc { Left,
	Right };

enum OpProps { OpPropsSymbol = 0,
	OpPropsPrec,
	OpPropsArity,
	OpPropsAssoc };

enum class ParenType { Left,
	Right };
...
struct op_token final : public expression_token {
	op_token(OpType o) :
		...
		switch (op) {
			case OpType::Add:
				props = std::make_tuple('+', 0, OpArity::Binary,
						OpAssoc::Left);
				break;
			case OpType::Sub:
				props = std::make_tuple('-', 0, OpArity::Binary,
				...
				break;
			case OpType::Mul:
				props = std::make_tuple('*', 1, OpArity::Binary,
						OpAssoc::Left);
				break;
			case OpType::Div:
				props = std::make_tuple('/', 1, OpArity::Binary,
			...
			case OpType::Mod:
				...
				break;
			case OpType::UPlus:
				props = std::make_tuple('+', 2, OpArity::Unary,
						OpAssoc::Right);
				break;
			case OpType::UMinus:
				props = std::make_tuple('-', 2, OpArity::Unary,
				...
				break;
			case OpType::Exp:
				props = std::make_tuple('^', 2, OpArity::Binary,
						OpAssoc::Right);
				break;
		}
		...
		switch (token->type()) {
			...
			case TokenType::Operator: {
				...
				while (!op_stack.empty()) {
					...
					auto &op2 = static_cast<const op_token &>(
							*op_stack.top());
					if ((OpAssoc::Left == op1.assoc &&
								op1 <= op2) ||
							(OpAssoc::Right == op1.assoc &&
									op1 < op2)) {
...
expression::token_vector tokenize_infix(const str_t &infix_string) {
	...
	while (ss >> c) {
		...
		// Right Parenthesis
			...
			if (TokenType::Operator == previous) {
				...
				auto &p = static_cast<const paren_token &>(
						*tokens.back());
				if (ParenType::Left == p.paren) {
					tokens.clear();
					throw runtime_error(syntax_error);
		...
		// Variable
			...
			if (is_variable(var)) {
				...
				throw runtime_error(
						"tokenize_infix : invalid variable name");
			}
		}
		// Invalid
		else {
	...
	if (tokens.empty()) {
		throw runtime_error(
				"tokenize_infix : expression has no tokens");
	} else {
...
real_t expression::evaluate(const var_map &vars) const {
	...
	if (!missing_var.empty()) {
		throw runtime_error(
				msg + "expression depends on undefined variable, '" +
				missing_var + "'");
	}
