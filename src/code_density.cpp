...
void dnl_ramp(const uint64_t *hist_data, real_t *dnl_data, size_t size) {
	...
	real_t sum = 0.0;
	for (size_t i = nz.first + 1; i < nz.second;
			++i) { // exclude first and last NZ bins
...
void dnl_tone(const uint64_t *hist_data, real_t *dnl_data, size_t size) {
	...
	for (size_t i = 1; i < size; ++i) {
		dnl_data[i] =
				dnl_data[i - 1] + static_cast<real_t>(hist_data[i]);
	}
	...
	for (size_t i = nz.first + 1; i <= median_index; ++i) {
		if (peak <
				hist_data[i]) { // Note the less-than: if multiple bins contain the
	...
	for (size_t i = median_index; i <= nz.second; ++i) {
		if (peak <=
				hist_data[i]) { // Note the less-than-or-equal: if multiple bins
			peak = hist_data
	...
	if (!(left_peak_index < median_index &&
				median_index < right_peak_index)) {
		std::fill(dnl_data, dnl_data + size, -1.0);
	...
	if (nz.first == left_dnl_index) { // exclude first NZ bin
		left_dnl_index +=
				1; // certain: 0 < left_dnl_index <= median_index
	}
	...
	if (avg_code_width <=
			0.0) { // not sure how this can happen, but just in case...
...
size_t code_density_size(int n, CodeFormat format) {
	std::pair<int64_t, int64_t> mm =
			resolution_to_minmax<int64_t>(n, format);
...
void dnl(real_t *dnl_data, size_t dnl_size, const uint64_t *hist_data,
	...
	check_array_pair("dnl : ", "hist array", hist_data, hist_size,
			"dnl array", dnl_data, dnl_size);
	switch (type) {
		case DnlSignal::Ramp:
...
std::map<str_t, real_t> dnl_analysis(const real_t *data, size_t size) {
	...
	if (first_nm_index < size) { // if there are any non-missing codes
		...
		num_codes = 1.0 +
				static_cast<real_t>(last_nm_index - first_nm_index);
	} else {
...
template void hist(uint64_t *, size_t, const int16_t *, size_t, int, CodeFormat,
		bool);
template void hist(uint64_t *, size_t, const int32_t *, size_t, int, CodeFormat,
		bool);
template void hist(uint64_t *, size_t, const int64_t *, size_t, int, CodeFormat,
		bool);
...
std::map<str_t, real_t> hist_analysis(const uint64_t *data, size_t size) {
	...
	if (first_nz_index < size) { // if there are any non-zero bins
		...
		num_bins = 1.0 +
				static_cast<real_t>(last_nz_index - first_nz_index);
	} else {
...
void inl(real_t *inl_data, size_t inl_size, const real_t *dnl_data,
	...
	std::partial_sum(dnl_data + first, dnl_data + last + 1,
			inl_data + first);
	real_t m = 0.0;
	real_t b = 0.0;
	switch (fit) {
