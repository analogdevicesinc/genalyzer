...
fourier_analysis_results fourier_analysis::analyze(const real_t *in_data,
	...
	if (in_size == nfft || // Real data, complex analysis
		...
		msq_size = in_size;
			...
			in_size ==
					(nfft / 2 + 1) * 2) { // Complex data, real analysis
	...
	fourier_analysis_results results = analyze_impl(
			msq_data, msq_size, nfft, axis_type, fft_data, fft_size);
	return results;
}
...
void fourier_analysis::add_fixed_tone(const str_t &key, FACompTag tag,
	...
	m_user_comps.insert(
			{ key, std::make_unique<fa_fixed_tone>(tag, fe.to_string(FPFormat::Eng), ssb) });
}

void fourier_analysis::add_max_tone(const str_t &key, FACompTag tag,
	...
	if (FACompTag::Signal == tag || FACompTag::UserDist == tag ||
		...
		m_user_keys.push_back(key);
		m_user_comps.insert(
				{ key, std::make_unique<fa_max_tone>(tag, ce.to_string(FPFormat::Eng), we.to_string(FPFormat::Eng), ssb) });
	} else {
		throw runtime_error(
				"tag must be one of {" +
...
void fourier_analysis::remove_comp(const str_t &key) {
	if (is_comp(key)) {
		...
		m_user_keys.erase(std::remove(m_user_keys.begin(),
								  m_user_keys.end(), key),
				m_user_keys.end());
	}
}
...
void fa_set_expr(const str_t &name, str_t &m_expr, const str_t &expr,
...
void fourier_analysis::set_analysis_band(const str_t &center,
		const str_t &width) {
...
str_t fourier_analysis::preview(bool cplx) const {
	...
	std::vector<str_vector> header{ { "Index", "Key", "Type", "Tag",
			"Spec" } };
...
bool is_cplx_analysis(size_t in_size, size_t nfft) {
	...
	if (in_size == nfft || in_size == nfft * 2) {
		cplx = true;
	} else if (!(in_size == nfft / 2 + 1 ||
					   in_size == (nfft / 2 + 1) * 2)) {
		throw runtime_error(
				"Invalid combination of data size and NFFT");
	}
	return cplx;
}
...
std::vector<size_t> fourier_analysis::result_key_lengths(size_t in_size,
		size_t nfft) const {
...
void fourier_analysis::add_comp(str_vector &keys, comp_map &comps,
		const str_t &k, comp_ptr c) {
...
fourier_analysis::comp_data_t fourier_analysis::generate_comps(bool cplx) const {
	...
	for (const fraction &f : ilos_clk_terms) {
		...
		if (ilv_terms.end() != ilv_terms.find(f)) {
			...
			add_comp(keys, comps, key,
					fa_fixed_tone::create(FACompTag::ILOS, key,
							m_ssb_def));
			if (is_also_clk) {
			...
			if (cplx && "fs/2" != key) {
				key.insert(0, 1, '-');
				add_comp(keys, comps, key,
						fa_fixed_tone::create(FACompTag::ILOS,
								key, m_ssb_def));
				if (is_also_clk) {
					ilos_clk_keys.insert(key);
				}
			...
			add_comp(keys, comps, key,
					fa_fixed_tone::create(FACompTag::CLK, key,
							m_ssb_def));
			if (cplx && "fs/2" != key) {
				key.insert(0, 1, '-');
				add_comp(keys, comps, key,
						fa_fixed_tone::create(FACompTag::CLK,
								key, m_ssb_def));
			}
		}
	}
	...
	for (const str_t &ukey : m_user_keys) {
		...
		if (FACompType::FixedTone == comp->type) {
			const auto &c =
					static_cast<const fa_fixed_tone &>(*comp);
			...
			ssb = (c.ssb < 0) ? def_user_ssb : c.ssb;
			add_comp(keys, comps, ukey,
					fa_max_tone::create(c.tag, c.center, c.width,
							ssb));
		} else {
			throw runtime_error(
		...
		if (find_fi) {
			key = '-' + ukey;
			add_comp(keys, comps, key,
					fa_fixed_tone::create(FACompTag::HD, key,
							ssb));
		}
		...
		for (int i = 2; i <= m_hd; ++i) {
			if (is_even(i)) {
				...
				add_comp(keys, comps, key,
						fa_fixed_tone::create(FACompTag::HD,
								key, ssb));
				if (cplx) {
					key.insert(0, 1, '-');
					add_comp(keys, comps, key,
		...
		for (const fraction &f : ilv_terms) {
			key = ukey + '+' + f.term;
			add_comp(keys, comps, key,
					fa_fixed_tone::create(FACompTag::ILGT, key,
							ssb));
			if ("fs/2" != f.term) {
				key = ukey + '-' + f.term;
				add_comp(keys, comps, key,
						fa_fixed_tone::create(FACompTag::ILGT,
								key, ssb));
			}
			if (find_qe) {
...
bool check_args(const real_t *msq_data, const size_t msq_size,
		const size_t nfft, const cplx_t *fft_data,
...
real_t fa_phase(const fa_tone_results &r, const cplx_t *fft_data, bool cplx,
	...
	if (fft_data && 1 == r.get(FAToneResult::NBins)) {
		...
		if (!cplx) {
			...
			if (fdata <= 2 * f) {
				phase *=
						-1.0; // invert phase if falias is in 2nd NZ
			}
		}
	}
	return phase;
...
diff_p get_lrbins(size_t nfft, bool cplx, real_t cycles, int ssb) {
	...
	if (0 < ssb) { // generate_comps() guarantees 0 <= ssb
		...
		nearest_half_cycle = std::round(
				nearest_half_cycle); // nearest whole cycle (x.0)
	}
	...
	if (is_even(nfft) &&
			!on_half_cycle) { // Example( NFFT=32, !on_half_cycle ):
		max_ssb -=
				1; //             ssb=16 -> 1+2*16=33 bins - too many!
...
fa_tone_results meas_dc(const fa_dc &comp, const real_t *msq_data,
		const size_t msq_size, const size_t nfft,
...
fa_tone_results meas_fixed_tone(const str_t &key, const fa_fixed_tone &comp,
...
fa_tone_results meas_max_tone(const str_t &key, const fa_max_tone &comp,
		const real_t *msq_data, const size_t msq_size,
		const size_t nfft,
		fourier_analysis::mask_map &masks,
		fourier_analysis::var_map &vars) {
	...
	diff_p lrbins = get_lrbins(nfft, cplx, static_cast<real_t>(max_index),
			comp.ssb);
...
fa_tone_results meas_wo_tone(const fa_wo_tone &comp, const real_t *msq_data,
		const size_t msq_size, const size_t nfft,
		const real_t fbin, const real_t fshift,
		fourier_analysis::mask_map &masks) {
...
void update_maxspur(size_t key_index, const fa_tone_results &results,
...
void tone_updates(const str_t &key, size_t key_index, FACompTag tag,
	...
	if (ilos_clk_keys.find(key) != ilos_clk_keys.end()) {
		masks.at(to_int(FACompTag::CLK))
				.set_range(results.i1, results.i2);
	}
	if (results.inband) {
		if (FACompTag::Signal == tag) {
			...
			update_maxspur(key_index, results, dc_as_dist, masks,
					maxspur_im);
		}
	}
}
...
fourier_analysis::mask_map fourier_analysis::initialize_masks(bool cplx,
		size_t size) {
	mask_map masks{
		{ to_int(FAMask::AB), fourier_analysis_comp_mask(cplx, size) },
		{ to_int(FAMask::Comp),
				fourier_analysis_comp_mask(cplx, size) },
...
fourier_analysis::analyze_impl(const real_t *msq_data, const size_t msq_size,
	...
	//
	const bool cplx =
			check_args(msq_data, msq_size, nfft, fft_data, fft_size);
	...
	for (; key_index < keys.size(); ++key_index) {
		...
		switch (comp.type) {
			case FACompType::DC: {
				...
				fa_tone_results r =
						meas_dc(c, msq_data, msq_size, nfft, masks);
				if (r.inband && dc_as_dist) {
					update_maxspur(key_index, r, dc_as_dist, masks,
							maxspur_im);
				}
			...
			case FACompType::FixedTone: {
				...
				fa_tone_results r = meas_fixed_tone(
						key, c, msq_data, msq_size, nfft, masks, vars);
				tone_updates(key, key_index, comp.tag, r, dc_as_dist,
						masks, ilos_clk_keys, carrier_im,
						maxspur_im);
				results.add_tone(key, std::move(r));
				break;
			}
			case FACompType::MaxTone: {
				...
				fa_tone_results r = meas_max_tone(
						key, c, msq_data, msq_size, nfft, masks, vars);
				tone_updates(key, key_index, comp.tag, r, dc_as_dist,
						masks, ilos_clk_keys, carrier_im,
						maxspur_im);
	...
	fourier_analysis_comp_mask &wo_mask = masks.at(to_int(FAMask::WO));
	wo_mask =
			masks.at(to_int(FAMask::AB)); // Initialize WO mask with AB mask
	wo_mask.unset_ranges(masks.at(to_int(
			FAMask::Comp))); // Then remove all components already found
	const size_t first_wo_index = key_index;
	for (; key_index < keys.size(); ++key_index) {
		const str_t &key = keys[key_index];
		auto &c = static_cast<const fa_wo_tone &>(*comps.at(key));
		fa_tone_results r = meas_wo_tone(c, msq_data, msq_size, nfft,
				fbin, fshift, masks);
	...
	// MaxSpur.
	int wo_num = 0;
	for (auto iter = wo_mag_map.crbegin(); iter != wo_mag_map.crend();
			++iter) {
		...
		if (1 == m_wo || 1 == wo_num) {
			update_maxspur(first_wo_index, wo_res_map.at(key),
					dc_as_dist, masks, maxspur_im);
		}
	...
	if (0 < carrier_im.first) {
		...
		fa_tone_results &carrier_results =
				results.tone_results.at(carrier_key);
		carrier_phase = fa_phase(carrier_results, fft_data, cplx, fdata,
				fshift);
	}
	for (size_t i = 0; i < keys.size(); ++i) {
		...
		if (has_tone_results(*comps.at(key))) {
			...
			real_t phase =
					fa_phase(r, fft_data, cplx, fdata, fshift);
	...
	if (cplx && FreqAxisType::DcCenter == axis_type) {
		for (std::pair<const str_t, fa_tone_results> &kv :
				results.tone_results) {
			double &ffinal =
					kv.second.results[FAToneResult::FFinal];
			if (fdata <= 2 * ffinal) {
				ffinal -= fdata;
			}
		}
	...
	real_t noise_ss =
			masks.at(to_int(FAMask::Noise)).sum(msq_data, msq_size);
	...
	results.set(FAResult::SFDR,
			fa_db10(carrier_im.second, maxspur_im.second));
	results.set(FAResult::ABN,
			fa_db10(noise_ss /
					std::fmax(1.0, noise_nbins))); // avoid div by 0
	results.set(FAResult::NSD, fa_db10(noise_ss / ab_width));
	results.set(FAResult::CarrierIndex,
			static_cast<real_t>(carrier_im.first));
	results.set(FAResult::MaxSpurIndex,
			static_cast<real_t>(maxspur_im.first));
	...
	results.set(FAResult::UserDist_RSS,
			masks.at(to_int(FACompTag::UserDist))
					.root_sum(msq_data, msq_size));
...
void fourier_analysis::finalize_masks(mask_map &masks) const {
	const fourier_analysis_comp_mask &ab_mask =
			masks.at(to_int(FAMask::AB));
	// Signal Mask: remove out-of-band Signals
	fourier_analysis_comp_mask &sig_mask =
			masks.at(to_int(FACompTag::Signal));
	sig_mask &= ab_mask;
	...
	for (const std::pair<int, str_t> &kv : fa_comp_tag_map) {
		if (to_int(FACompTag::Signal) !=
				kv.first) { // Don't touch the Signal mask!
	...
	if (dc_as_dist) {
		masks.at(to_int(FAMask::Dist)) |=
				masks.at(to_int(FACompTag::DC));
	}
	if (!clk_as_noise) {
		masks.at(to_int(FAMask::Dist)) |=
				masks.at(to_int(FACompTag::CLK));
...
void fourier_analysis::setup_analysis_band(bool cplx,
