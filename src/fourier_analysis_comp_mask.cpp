...
fourier_analysis_comp_mask::fourier_analysis_comp_mask(
		...
		m_mode{ cplx ? Wrap : Stop }, m_ufirst{ 0 }, m_ulast{ array_size - 1 }, m_usize{ array_size }, m_first{ 0 }, m_last{ static_cast<diff_t>(array_size - 1) }, m_size{ static_cast<diff_t>(array_size) }, m_data(init) {
	if (!m_data.empty()) {
		const char *msg =
				"fourier_analysis_comp_mask : invalid construction";
...
fourier_analysis_comp_mask::operator|=(const fourier_analysis_comp_mask &m) {
	...
	for (size_t range = 0; range < m.num_ranges(); ++range) {
		...
		this->set_range_safe(
				i1, i2 - 1); // subtract 1 because i2 is 1-past!
	}
	return *this;
}
...
fourier_analysis_comp_mask::find_max_index(const real_t *data,
	...
	if (max_index < 0) {
		...
		diff_t upper =
				static_cast<diff_t>(m_data[max_range * 2 + 1]) - 1;
...
fourier_analysis_comp_mask::get_indexes() const {
	if (1 == num_ranges()) {
		return std::make_tuple(m_data[0], m_data[1] - 1,
				m_data[1] - m_data[0]);
	} else if (2 == num_ranges() && m_ufirst == m_data[0] &&
			m_usize == m_data[3]) {
...
bool fourier_analysis_comp_mask::overlaps(size_t left, size_t right) const {
	...
	if (is_odd(left_index) ||
			is_odd(right_index)) { // if either are inside a range
		return true;
	}
	return left_index !=
...
void fourier_analysis_comp_mask::set_range(diff_t left, diff_t right) {
	...
	if (Stop == m_mode) {
		...
		if (m_last < right) {
			...
			if (m_last < right) {
				throw runtime_error(
						"fourier_analysis_comp_mask::set_range : right out of range");
			}
		}
...
real_t fourier_analysis_comp_mask::sum(const real_t *data, size_t size) const {
	if (m_usize != size) {
		throw runtime_error(
				"fourier_analysis_comp_mask::sum : size error");
	}
	real_t mag = 0.0;
	for (size_t range = 0; range < num_ranges(); ++range) {
		const size_t i1 = m_data[range * 2];
...
void fourier_analysis_comp_mask::unset_ranges(
...
void fourier_analysis_comp_mask::if_not_compat_then_throw(
		const fourier_analysis_comp_mask &m) {
